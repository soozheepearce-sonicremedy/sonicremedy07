...
const PLAYLIST = [
  ...
  { num: 4, title: 'Inhale Gratitude Exhale Life Support', full: 'Inhale Gratitude Exhale Life Support', cover: 'assets/covers/af7-cover-04.jpg', track: 'assets/tracks/af7-04.mp3' },
...
function loadTrack(index, autoplay = false) {
  ...
  $('nowPlayingNum').textContent = 'Track ' + String(t.num).padStart(2, '0') + ' of 16';
...
function toggleFormulaMenu() {
  const button = $('formulaMenuButton');
...
function closeFormulaMenu() {
  const button = $('formulaMenuButton');
  const menu = $('formulaMenu');
  if (button && menu) { menu.classList.remove('open'); button.setAttribute('aria-expanded', 'false'); }
}
document.addEventListener('click', function (e) {
  const button = $('formulaMenuButton');
  const selector = e.target.closest('.formula-selector');
  if (button && e.target.closest('#formulaMenuButton')) { e.preventDefault(); toggleFormulaMenu(); }
  else if (!selector) closeFormulaMenu();
});
